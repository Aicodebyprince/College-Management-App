...
class _AcademicCalendarState extends State<AcademicCalendar> {
  ...
  Future<void> addTaskToFirebase(CustomTask task) async {
  ...
  Future<void> deleteTaskFromFirebase(CustomTask task) async {
    CollectionReference tasks = FirebaseFirestore.instance.collection('tasks');

    // Fetch the document ID for the task
    QuerySnapshot snapshot =
        await tasks
            .where('fromDate', isEqualTo: task.fromDate)
  ...
  Future<void> fetchTasksFromFirebase() async {
    ...
    try {
      ...
      _showErrorDialog("Failed to fetch tasks from Firebase: $e");
    }
  }
  ...
  void _showAddTaskDialog() {
    ...
    // Use a StatefulBuilder to update dialog's internal state.
    showDialog(
      context: context,
      builder: (context) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ListTile(
                        ...
                        trailing: const Icon(Icons.calendar_today),
                        onTap: () async {
                          DateTime initialDate = fromDate ?? selectedDate;
                          final picked = await showDatePicker(
                            context: context,
                            initialDate: initialDate,
                            firstDate: DateTime(2000),
                ...
                ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate() &&
                        ...
                        startTime != null &&
                        endTime != null) {
                      // Validation for dates and times as before...
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        // The Delete/Confirm button.
                        if (selectedCustomTasks.isNotEmpty ||
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () async {
                              // In the "Delete/Confirm" button's onPressed:
                              if (!isDeleteMode) {
                                // Enter delete mode.
                                setState(() {
                                  isDeleteMode = true;
                                ...
                                if (tasksToDelete.isEmpty) {
                                  // Show an error dialog if no task is selected.
                                  showDialog(
                                    context: context,
                                    builder: (context) {
                                      return AlertDialog(
                                ...
                                // If tasks are selected, proceed with confirmation.
                                bool confirm =
                                ...
                                if (confirm) {
                                  ...
                                  for (CustomTask task in tasksToDelete) {
                                    await deleteTaskFromFirebase(task);
                                  }

                                  setState(() {
                                    customTasks.removeWhere(
                                      (task) => tasksToDelete.contains(task),
                                    );
                                    isDeleteMode = false;
                                    tasksToDelete.clear();
                                  });
